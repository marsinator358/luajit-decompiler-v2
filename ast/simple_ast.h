...
private:
	...
	struct SlotScope;
	struct Function;
	#include "simple_building_blocks.h"

public:

	SimpleAst(const Bytecode& bytecode);
	~SimpleAst();
...
private:
	...
	Statement*& new_statement(const AST_SIMPLE_STATEMENT& type);
	Expression*& new_expression(const AST_SIMPLE_EXPRESSION& type);
	void build_functions(Function& function);
	void build_instructions(Function& function);
	void assign_debug_info(Function& function);
	void group_jumps(Function& function, std::vector<Statement*>& block);
	void build_loops(Function& function, std::vector<Statement*>& block);
