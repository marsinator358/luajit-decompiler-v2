...
Ast::SimpleAst::SimpleAst(const Bytecode& bytecode) : bytecode(bytecode), chunk(*bytecode.main) {}

Ast::SimpleAst::~SimpleAst() {
...
Ast::SimpleAst::Function*& Ast::SimpleAst::new_function(const Bytecode::Prototype& prototype) {
	functions.emplace_back(new Function(prototype));
	return functions.back();
}

Ast::SimpleAst::Statement*& Ast::SimpleAst::new_statement(const AST_SIMPLE_STATEMENT& type) {
	statements.emplace_back(new Statement(type));
	return statements.back();
}

Ast::SimpleAst::Expression*& Ast::SimpleAst::new_expression(const AST_SIMPLE_EXPRESSION& type) {
	expressions.emplace_back(new Expression(type));
	return expressions.back();
}

void Ast::SimpleAst::build() {
...
void Ast::SimpleAst::build_functions(Function& function) {
...
void Ast::SimpleAst::build_instructions(Function& function) {
	...
	for (uint32_t i = function.block.size(); i--;) {
		function.block[i] = new_statement(AST_SIMPLE_STATEMENT_INSTRUCTION);
		...
		case Bytecode::BC_OP_RET1:
			function.block[i]->type = AST_SIMPLE_STATEMENT_RETURN;
			continue;
	...
	if (function.block.back()->instruction.type == Bytecode::BC_OP_RET0) function.block.back()->type = AST_SIMPLE_STATEMENT_EMPTY;
	return assign_debug_info(function);
}

void Ast::SimpleAst::assign_debug_info(Function& function) {
	if (!function.hasDebugInfo) return group_jumps(function, function.block);
	...
	for (uint32_t i = function.parameterNames.size(); i < function.prototype.variableInfos.size(); i++) {
		assert(!function.prototype.variableInfos[i].isParameter, "Function parameter count does not\nmatch with debug info", bytecode.filePath, DEBUG_INFO);
		while (activeLocalScopes.size() && function.prototype.variableInfos[i].scopeBegin > activeLocalScopes.back()) activeLocalScopes.pop_back();
	...
	return group_jumps(function, function.block);
}

void Ast::SimpleAst::group_jumps(Function& function, std::vector<Statement*>& block) {
	for (uint32_t i = block.size(); i--;) {
		if (block[i]->type != AST_SIMPLE_STATEMENT_INSTRUCTION) continue;

		switch (block[i]->instruction.type) {
		case Bytecode::BC_OP_UCLO:
			function.slotScopeCollector.add_upvalue_close(block[i]->instruction.info.id, block[i]->instruction.target, block[i]->instruction.a);

			if (block[i]->instruction.d == Bytecode::BC_OP_JMP_BIAS) {
				block[i]->type = AST_SIMPLE_STATEMENT_EMPTY;
				continue;
			}

			block[i]->type = AST_SIMPLE_STATEMENT_GOTO;
			function.add_jump(block[i]->instruction.info.id, block[i]->instruction.target);
			continue;
		case Bytecode::BC_OP_JMP:
			if (block[get_block_index_from_id(block, block[i]->instruction.target)]->instruction.type == Bytecode::BC_OP_ITERC) continue;

			if (block[i]->instruction.target != block[i]->instruction.info.id && i) {
				switch (block[i - 1]->instruction.type) {
				case Bytecode::BC_OP_JMP:
					if (block[i - 1]->instruction.d != Bytecode::BC_OP_JMP_BIAS) break;
				...
				case Bytecode::BC_OP_ISF:
					block[i - 1]->type = AST_SIMPLE_STATEMENT_CONDITION;
					block[i - 1]->condition.jumpId = block[i]->instruction.info.id;
					block[i - 1]->instruction.target = block[i]->instruction.target;
					block.erase(block.begin() + i);
					i--;
					function.slotScopeCollector.add_jump(block[i]->condition.jumpId, block[i]->instruction.target);
					function.add_jump(block[i]->condition.jumpId, block[i]->instruction.target);
					continue;
				}
			}

			block[i]->type = AST_SIMPLE_STATEMENT_GOTO;
			function.slotScopeCollector.add_jump(block[i]->instruction.info.id, block[i]->instruction.target);
			function.add_jump(block[i]->instruction.info.id, block[i]->instruction.target);
			continue;
		}
	}
	...
	return build_loops(function, block);
}

void Ast::SimpleAst::build_loops(Function& function, std::vector<Statement*>& block) {
	static void (* const build_break_statements)(std::vector<Statement*>&, const uint32_t&, const uint32_t&) =
		[](std::vector<Statement*>& block, const uint32_t& breakTarget, const uint32_t& secondaryBreakTarget)->void {
		for (uint32_t i = block.size(); i--;) {
			if (block[i]->type != AST_SIMPLE_STATEMENT_GOTO
				|| (block[i]->instruction.target != breakTarget
					&& block[i]->instruction.target != secondaryBreakTarget))
				continue;
			block[i]->type = AST_SIMPLE_STATEMENT_BREAK;
		}
	};
	...
	for (uint32_t i = block.size(); i--;) {
		...
		case Bytecode::BC_OP_ISNEXT:
		case Bytecode::BC_OP_JMP:
			block[i]->type = AST_SIMPLE_STATEMENT_GENERIC_FOR;
			targetIndex = get_block_index_from_id(block, block[i]->instruction.target);
			breakTarget = get_extended_id_from_statement(block[targetIndex + 2]);
			block[i]->instruction = block[targetIndex]->instruction;
			block[i]->instruction.info.id = block[targetIndex + 1]->instruction.target - 1;
			block[i]->instruction.target = block[targetIndex + 1]->instruction.info.id + 1;
			block[targetIndex]->type = AST_SIMPLE_STATEMENT_EMPTY;
			block[i]->block.reserve(targetIndex - i);
			block[i]->block.insert(block[i]->block.begin(), block.begin() + i + 1, block.begin() + targetIndex + 1);
			block.erase(block.begin() + i + 1, block.begin() + targetIndex + 2);
			function.slotScopeCollector.add_loop(block[i]->instruction.info.id, block[i]->instruction.target);
			build_break_statements(block[i]->block, breakTarget, block[i]->instruction.target);
			build_local_scopes(function, block[i]->block);
			continue;
		case Bytecode::BC_OP_FORI:
			block[i]->type = AST_SIMPLE_STATEMENT_NUMERIC_FOR;
			targetIndex = get_block_index_from_id(block, block[i]->instruction.target);
			breakTarget = get_extended_id_from_statement(block[targetIndex]);
			block[targetIndex - 1]->type = AST_SIMPLE_STATEMENT_EMPTY;
			block[i]->block.reserve(targetIndex - 1 - i);
			block[i]->block.insert(block[i]->block.begin(), block.begin() + i + 1, block.begin() + targetIndex);
			block.erase(block.begin() + i + 1, block.begin() + targetIndex);
			function.slotScopeCollector.add_loop(block[i]->instruction.info.id, block[i]->instruction.target);
			build_break_statements(block[i]->block, breakTarget, block[i]->instruction.target);
			build_local_scopes(function, block[i]->block);
			continue;
		case Bytecode::BC_OP_LOOP:
			assert(block[i]->instruction.target >= block[i]->instruction.info.id, "LOOP instruction has invalid jump target", bytecode.filePath, DEBUG_INFO);

			if (block[i]->instruction.target == block[i]->instruction.info.id) {
				assert(i + 1 < block.size() && block[i + 1]->type == AST_SIMPLE_STATEMENT_GOTO, "JMP instruction after goto loop is missing", bytecode.filePath, DEBUG_INFO);
				assert(block[i + 1]->instruction.target <= block[i]->instruction.info.id, "JMP instruction after goto loop has invalid target", bytecode.filePath, DEBUG_INFO);
				block[i]->type = AST_SIMPLE_STATEMENT_EMPTY;
				continue;
			}

			block[i]->type = AST_SIMPLE_STATEMENT_LOOP;
			targetIndex = get_block_index_from_id(block, block[i]->instruction.target);
			breakTarget = get_extended_id_from_statement(block[targetIndex]);
			block[i]->block.reserve(targetIndex - 1 - i);
			block[i]->block.insert(block[i]->block.begin(), block.begin() + i + 1, block.begin() + targetIndex);
			...
			if (block[i]->block.size()
				&& block[i]->block.back()->type == AST_SIMPLE_STATEMENT_CONDITION
				&& block[i]->block.back()->instruction.target != breakTarget
				&& block[i]->block.back()->instruction.target != block[i]->instruction.target) {
				assert(block[i]->block.back()->instruction.target == block[i]->instruction.info.id, "Repeat loop condition has invalid jump target", bytecode.filePath, DEBUG_INFO);
				block[i]->block.emplace_back(new_statement(AST_SIMPLE_STATEMENT_BREAK));
				block[i]->block.back()->instruction.type = Bytecode::BC_OP_JMP;
				block[i]->block.back()->instruction.target = block[i]->instruction.target;
				block[i]->block.emplace_back(new_statement(AST_SIMPLE_STATEMENT_GOTO));
				block[i]->block.back()->instruction.type = Bytecode::BC_OP_JMP;
				block[i]->block.back()->instruction.target = block[i]->instruction.info.id;
			}
	...
	return build_local_scopes(function, block);
}

void Ast::SimpleAst::build_local_scopes(Function& function, std::vector<Statement*>& block) {
	...
	for (uint32_t i = function.locals.size(); i--;) {
		...
		case AST_SIMPLE_STATEMENT_NUMERIC_FOR:
		case AST_SIMPLE_STATEMENT_GENERIC_FOR:
			block[scopeBeginIndex]->locals = &function.locals[i];
			continue;
		}

		scopeBeginIndex++;
		block.emplace(block.begin() + scopeBeginIndex, new_statement(AST_SIMPLE_STATEMENT_DECLARATION));
		block[scopeBeginIndex]->locals = &function.locals[i];
...
/*
void Ast::SimpleAst::remove_unreachable_instructions(Function& function, std::vector<Statement*>& block) {
	uint32_t nextReachableInstruction;

	for (uint32_t i = 0; i < block.size(); i++) {
		switch (block[i]->type) {
		case AST_SIMPLE_STATEMENT_GOTO:
		case AST_SIMPLE_STATEMENT_BREAK:
			...
			for (uint32_t j = nextReachableInstruction - 1; j > i; j--) {
				switch (block[j]->type) {
				case AST_SIMPLE_STATEMENT_CONDITION:
				...
				case AST_SIMPLE_STATEMENT_NUMERIC_FOR:
				case AST_SIMPLE_STATEMENT_GENERIC_FOR:
				case AST_SIMPLE_STATEMENT_LOOP:
					for (std::vector<std::vector<Statement*>*> callstack(1, &block[i]->block); callstack.size();) {
						...
						case AST_SIMPLE_STATEMENT_GENERIC_FOR:
						case AST_SIMPLE_STATEMENT_LOOP:
							callstack.back()->front()->type = AST_SIMPLE_STATEMENT_EMPTY;
...
void Ast::SimpleAst::build_expressions(Function& function, std::vector<Statement*>& block) {
	for (uint32_t i = block.size(); i--;) {
		block[i]->instruction.info.attachedLabel = function.get_label_from_id(block[i]->instruction.info.id);

		switch (block[i]->type) {
		case AST_SIMPLE_STATEMENT_INSTRUCTION:
			block[i]->type = AST_SIMPLE_STATEMENT_ASSIGNMENT;
			...
			case Bytecode::BC_OP_LEN:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_UNARY_OPERATION);

				switch (block[i]->instruction.type) {
				case Bytecode::BC_OP_NOT:
					block[i]->assignment.expressions.back()->unaryOperation->type = AST_SIMPLE_UNARY_NOT;
					break;
				case Bytecode::BC_OP_UNM:
					block[i]->assignment.expressions.back()->unaryOperation->type = AST_SIMPLE_UNARY_MINUS;
					break;
				case Bytecode::BC_OP_LEN:
					block[i]->assignment.expressions.back()->unaryOperation->type = AST_SIMPLE_UNARY_LENGTH;
					break;
				}
			...
			case Bytecode::BC_OP_CAT:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_BINARY_OPERATION);

				switch (block[i]->instruction.type) {
				case Bytecode::BC_OP_ADDVN:
				case Bytecode::BC_OP_ADDNV:
				case Bytecode::BC_OP_ADDVV:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_ADDITION;
					break;
				case Bytecode::BC_OP_SUBVN:
				case Bytecode::BC_OP_SUBNV:
				case Bytecode::BC_OP_SUBVV:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_SUBTRACTION;
					break;
				case Bytecode::BC_OP_MULVN:
				case Bytecode::BC_OP_MULNV:
				case Bytecode::BC_OP_MULVV:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_MULTIPLICATION;
					break;
				case Bytecode::BC_OP_DIVVN:
				case Bytecode::BC_OP_DIVNV:
				case Bytecode::BC_OP_DIVVV:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_DIVISION;
					break;
				case Bytecode::BC_OP_MODVN:
				case Bytecode::BC_OP_MODNV:
				case Bytecode::BC_OP_MODVV:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_MODULO;
					break;
				case Bytecode::BC_OP_POW:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_EXPONENTATION;
					break;
				case Bytecode::BC_OP_CAT:
					block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_CONCATENATION;
					break;
				}
				...
				case Bytecode::BC_OP_CAT:
					...
					for (Expression* expression = block[i]->assignment.expressions.back(); true; expression = expression->binaryOperation->rightOperand) {
						...
						expression->binaryOperation->rightOperand = new_expression(AST_SIMPLE_EXPRESSION_BINARY_OPERATION);
						expression->binaryOperation->rightOperand->binaryOperation->type = AST_SIMPLE_BINARY_CONCATENATION;
			...
			case Bytecode::BC_OP_KNIL: //TODO
				while (true) {
					block[i]->assignment.expressions.resize(1, nullptr);
					block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
					block[i]->assignment.expressions.back()->constant->type = AST_SIMPLE_CONSTANT_NIL;
					block[i]->assignment.variables.resize(1);
					block[i]->assignment.variables.back().type = AST_SIMPLE_VARIABLE_SLOT;
					block[i]->assignment.variables.back().slot = block[i]->instruction.d;
					if (block[i]->instruction.d == block[i]->instruction.a) break;
					block.emplace(block.begin() + i, new_statement(AST_SIMPLE_STATEMENT_ASSIGNMENT));
				...
				for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
					block[i]->assignment.variables[j].type = AST_SIMPLE_VARIABLE_SLOT;
					block[i]->assignment.variables[j].slot = block[i]->instruction.a + j;
					block[i]->assignment.expressions[j] = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
					block[i]->assignment.expressions[j]->constant->type = AST_SIMPLE_CONSTANT_NIL;
				}

				continue;
				*/
			case Bytecode::BC_OP_UGET:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_VARIABLE);
				block[i]->assignment.expressions.back()->variable->type = AST_SIMPLE_VARIABLE_UPVALUE;
			...
			case Bytecode::BC_OP_USETP:
				block[i]->assignment.variables.resize(1);
				block[i]->assignment.variables.back().type = AST_SIMPLE_VARIABLE_UPVALUE;
				...
				continue;
			case Bytecode::BC_OP_FNEW:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_FUNCTION);
				block[i]->assignment.expressions.back()->function = block[i]->function;
				break;
			case Bytecode::BC_OP_TNEW:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_TABLE);
			...
			case Bytecode::BC_OP_GGET:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_VARIABLE);
				block[i]->assignment.expressions.back()->variable->type = AST_SIMPLE_VARIABLE_GLOBAL;
			...
			case Bytecode::BC_OP_GSET:
				block[i]->assignment.variables.resize(1);
				block[i]->assignment.variables.back().type = AST_SIMPLE_VARIABLE_GLOBAL;
			...
			case Bytecode::BC_OP_TGETB:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_VARIABLE);
				block[i]->assignment.expressions.back()->variable->type = AST_SIMPLE_VARIABLE_TABLE_INDEX;
			...
			case Bytecode::BC_OP_TSETB:
				block[i]->assignment.variables.resize(1);
				block[i]->assignment.variables.back().type = AST_SIMPLE_VARIABLE_TABLE_INDEX;
			...
			case Bytecode::BC_OP_CALL:
				if (block[i]->instruction.b) {
					if (block[i]->instruction.b == 1) {
						block[i]->type = AST_SIMPLE_STATEMENT_FUNCTION_CALL;
					} else {
				...
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_FUNCTION_CALL);
			...
			case Bytecode::BC_OP_VARG:
				...
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_VARARG);
				continue;
			}
			...
			block[i]->assignment.variables.back().slot = block[i]->instruction.a;
			continue;
		case AST_SIMPLE_STATEMENT_RETURN:
			switch (block[i]->instruction.type) {
			case Bytecode::BC_OP_CALLMT:
			case Bytecode::BC_OP_CALLT:
				block[i]->assignment.expressions.resize(1, nullptr);
		...
		case AST_SIMPLE_STATEMENT_CONDITION:
			...
			case Bytecode::BC_OP_ISNEP:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_BINARY_OPERATION);
				block[i]->assignment.expressions.back()->binaryOperation->type = AST_SIMPLE_BINARY_CONDITION;
			...
			case Bytecode::BC_OP_ISF:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_UNARY_OPERATION);
				block[i]->assignment.expressions.back()->unaryOperation->type = AST_SIMPLE_UNARY_CONDITION;
				block[i]->assignment.expressions.back()->unaryOperation->operand = new_slot(block[i]->instruction.d);
				block[i]->assignment.register_open_slots(block[i]->assignment.expressions.back()->unaryOperation->operand);
				break;
			case Bytecode::BC_OP_JMP:
				block[i]->assignment.expressions.back() = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
				block[i]->assignment.expressions.back()->constant->type = AST_SIMPLE_CONSTANT_CONDITION;
				break;
			}
		...
		case AST_SIMPLE_STATEMENT_GENERIC_FOR:
			...
			block[i]->assignment.register_open_slots(block[i]->assignment.expressions[0], block[i]->assignment.expressions[1], block[i]->assignment.expressions[2]);
			continue;
		case AST_SIMPLE_STATEMENT_DECLARATION:
...
void Ast::SimpleAst::collect_slot_scopes(Function& function, std::vector<Statement*>& block) {
	uint32_t id;
	...
	for (uint32_t i = block.size(); i--;) {
		...
		case AST_SIMPLE_STATEMENT_LOOP:
			function.slotScopeCollector.extend_scopes(block[i]->instruction.info.id - 1);
		...
		for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
			...
			case AST_SIMPLE_VARIABLE_SLOT:
				...
				continue;
			case AST_SIMPLE_VARIABLE_TABLE_INDEX:
		...
		if (block[i]->instruction.info.id != ID_EMPTY) {
			if (block[i]->instruction.info.attachedLabel != ID_EMPTY) {
...
void Ast::SimpleAst::eliminate_slots(Function& function, std::vector<Statement*>& block) {
	Expression* expression;

	for (uint32_t i = 0; i < block.size(); i++) {
		if (block[i]->condition.allowSlotSwap
			&& i >= 2
			&& block[i]->instruction.info.attachedLabel == ID_EMPTY
			&& block[i - 1]->instruction.info.attachedLabel == ID_EMPTY
			&& block[i - 1]->type == AST_SIMPLE_STATEMENT_ASSIGNMENT
			&& block[i - 1]->assignment.variables.size() == 1
			&& block[i - 1]->assignment.variables.back().type == AST_SIMPLE_VARIABLE_SLOT
			...
			&& block[i - 2]->type == AST_SIMPLE_STATEMENT_ASSIGNMENT
			&& block[i - 2]->assignment.variables.size() == 1
			&& block[i - 2]->assignment.variables.back().type == AST_SIMPLE_VARIABLE_SLOT
		...
		for (uint8_t j = block[i]->assignment.openSlots.size(); j--
			...
			if (j == 1
				...
				&& block[i - 1]->assignment.expressions.back()->type == AST_SIMPLE_EXPRESSION_VARIABLE
				&& block[i - 1]->assignment.expressions.back()->variable->type == AST_SIMPLE_VARIABLE_TABLE_INDEX
				&& block[i - 1]->assignment.expressions.back()->variable->table->type == AST_SIMPLE_EXPRESSION_VARIABLE
				&& block[i - 1]->assignment.expressions.back()->variable->table->variable->type == AST_SIMPLE_VARIABLE_SLOT
				&& block[i - 1]->assignment.expressions.back()->variable->tableIndex->type == AST_SIMPLE_EXPRESSION_CONSTANT
				&& block[i - 1]->assignment.expressions.back()->variable->tableIndex->constant->type == AST_SIMPLE_CONSTANT_STRING
				...
				&& block[i - 2]->assignment.expressions.back()->type == AST_SIMPLE_EXPRESSION_VARIABLE
				&& block[i - 2]->assignment.expressions.back()->variable->type == AST_SIMPLE_VARIABLE_SLOT
		...
		assert(!block[i]->assignment.openSlots.size()
			|| (*block[i]->assignment.openSlots.back())->type != AST_SIMPLE_EXPRESSION_VARIABLE
		...
		for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
			switch (block[i]->assignment.variables[j].type) {
			case AST_SIMPLE_VARIABLE_SLOT:
				function.slotScopeCollector.merge_locked_scope(block[i]->assignment.variables[j].slot, block[i]->assignment.variables[j].slotScope);
				continue;
			case AST_SIMPLE_VARIABLE_TABLE_INDEX:
				if (i
					&& block[i]->instruction.info.attachedLabel == ID_EMPTY
		...
		case AST_SIMPLE_STATEMENT_DECLARATION:
			for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
				assert(function.slotScopeCollector.lock_scope(block[i]->assignment.variables[j].slot, block[i]->assignment.variables[j].slotScope),
					"Tried to lock already locked scope", bytecode.filePath, DEBUG_INFO);
			}

			eliminate_slots(function, block[i]->block);

			for (uint8_t j = block[i]->assignment.variables.size(); j--;) {
				function.slotScopeCollector.unlock_scope(block[i]->assignment.variables[j].slot);
			}
...
/*struct Ast::SimpleAst::ConditionGrouper {
	...
	Statement* const& get_if_statement(const uint32_t& conditionStart, const uint32_t& conditionEnd) {
		Statement* const ifStatement = simpleAst.new_statement(AST_SIMPLE_STATEMENT_IF);
		...
		for (uint32_t i = conditionBlocks.size(); i--;) {
			...
			for (uint32_t j = conditionBlocks[i].conditionEnd - 1; j-- > conditionStop;) {
				if (statementBlock[j]->type == AST_SIMPLE_STATEMENT_CONDITION) {
					...
					conditionBlocks[i].condition.emplace_back(simpleAst.new_statement(
						isExitAtJumpTarget == (statementBlock[j]->instruction.jumpTargetId == conditionBlocks[i].jumpTargetId) ? AST_SIMPLE_STATEMENT_AND : AST_SIMPLE_STATEMENT_OR));
					...
					conditionBlocks[i].condition.front()->negated = conditionBlocks[i].condition.front()->type == AST_SIMPLE_STATEMENT_AND;
...
void Ast::SimpleAst::build_conditions(Function& function, std::vector<Statement*>& block) {
	//ConditionGrouper conditionGrouper(*this, currentBlock); uint32_t maxEndTarget, nextMaxEndTarget, endTarget;

	for (uint32_t i = 0; i < block.size(); i++) {
		/*if (currentBlock[i]->type != AST_SIMPLE_STATEMENT_CONDITION) continue;
		endTarget = i + 1;

		while (true) {
			//if (currentBlock[j]->type != AST_SIMPLE_STATEMENT_CONDITION || !conditionGrouper.add_condition(currentBlock[j]->instruction, j)) continue;
			...
			for (uint32_t j = endTarget; j < maxEndTarget; j++) {
				...
				case AST_SIMPLE_STATEMENT_INSTRUCTION:
				case AST_SIMPLE_STATEMENT_FUNCTION:
					continue;
				}
			...
			if (jumpTarget > endTarget
				&& currentBlock[jumpTarget - 1]->type == AST_SIMPLE_STATEMENT_CONDITION
...
uint32_t Ast::SimpleAst::get_block_index_from_id(const std::vector<Statement*>& block, const uint32_t& id) {
...
uint32_t Ast::SimpleAst::get_extended_id_from_statement(Statement* const& statement) {
...
void Ast::SimpleAst::check_valid_name(Constant* const& constant) {
...
void Ast::SimpleAst::check_special_number(Expression* const& expression, const bool& convertInfinity) {
	...
	expression->set_type(AST_SIMPLE_EXPRESSION_BINARY_OPERATION);
	expression->binaryOperation->type = AST_SIMPLE_BINARY_DIVISION;
	expression->binaryOperation->leftOperand = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->binaryOperation->leftOperand->constant->type = AST_SIMPLE_CONSTANT_NUMBER;
	expression->binaryOperation->leftOperand->constant->number = raw_double & DOUBLE_SIGN ? -1 : 1;
	expression->binaryOperation->rightOperand = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->binaryOperation->rightOperand->constant->type = AST_SIMPLE_CONSTANT_NUMBER;
	expression->binaryOperation->rightOperand->constant->number = 0;
}

Ast::SimpleAst::ConstantType Ast::SimpleAst::get_constant_type(Expression* const& expression) {
	...
	case AST_SIMPLE_EXPRESSION_CONSTANT:
		...
		case AST_SIMPLE_CONSTANT_NIL:
		case AST_SIMPLE_CONSTANT_FALSE:
			constantType.type = ConstantType::FALSY_CONSTANT;
			break;
		case AST_SIMPLE_CONSTANT_TRUE:
		case AST_SIMPLE_CONSTANT_STRING:
			constantType.type = ConstantType::TRUTHY_CONSTANT;
			break;
		case AST_SIMPLE_CONSTANT_NUMBER:
	...
	case AST_SIMPLE_EXPRESSION_BINARY_OPERATION:
		...
		case AST_SIMPLE_BINARY_ADDITION:
		case AST_SIMPLE_BINARY_SUBTRACTION:
		case AST_SIMPLE_BINARY_MULTIPLICATION:
		case AST_SIMPLE_BINARY_DIVISION:
		case AST_SIMPLE_BINARY_EXPONENTATION:
		case AST_SIMPLE_BINARY_MODULO:
		case AST_SIMPLE_BINARY_AND:
		case AST_SIMPLE_BINARY_OR:
			...
			case AST_SIMPLE_BINARY_MODULO:
				...
				break;
			case AST_SIMPLE_BINARY_AND: //TODO
				...
				break;
			case AST_SIMPLE_BINARY_OR: //TODO
				switch (constantType.type) {
				case ConstantType::FALSY_CONSTANT:
					constantType = get_constant_type(expression->binaryOperation->rightOperand);
					break;
...
Ast::SimpleAst::Expression* Ast::SimpleAst::new_slot(const uint8_t& slot) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_VARIABLE);
	expression->variable->type = AST_SIMPLE_VARIABLE_SLOT;
	expression->variable->slot = slot;
	return expression;
}

Ast::SimpleAst::Expression* Ast::SimpleAst::new_literal(const uint8_t& literal) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->constant->type = AST_SIMPLE_CONSTANT_NUMBER;
	expression->constant->number = literal;
	return expression;
}

Ast::SimpleAst::Expression* Ast::SimpleAst::new_signed_literal(const uint16_t& signedLiteral) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->constant->type = AST_SIMPLE_CONSTANT_NUMBER;
...
Ast::SimpleAst::Expression* Ast::SimpleAst::new_primitive(const uint16_t& primitive) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);

	switch (primitive) {
	case 0:
		expression->constant->type = AST_SIMPLE_CONSTANT_NIL;
		break;
	case 1:
		expression->constant->type = AST_SIMPLE_CONSTANT_FALSE;
		break;
	case 2:
		expression->constant->type = AST_SIMPLE_CONSTANT_TRUE;
		break;
	}
...
Ast::SimpleAst::Expression* Ast::SimpleAst::new_number(const Function& function, const uint16_t& index) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->constant->type = AST_SIMPLE_CONSTANT_NUMBER;
...
Ast::SimpleAst::Expression* Ast::SimpleAst::new_string(const Function& function, const uint16_t& index) {
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
	expression->constant->type = AST_SIMPLE_CONSTANT_STRING;
	expression->constant->string = function.get_constant(index).string;
	return expression;
}

Ast::SimpleAst::Expression* Ast::SimpleAst::new_table(const Function& function, const uint16_t& index) {
	static Expression* (* const new_table_constant)(SimpleAst&, const Bytecode::TableConstant&) = [](SimpleAst& simpleAst, const Bytecode::TableConstant& constant)->Expression* {
		Expression* expression = simpleAst.new_expression(AST_SIMPLE_EXPRESSION_CONSTANT);
		...
		case Bytecode::BC_KTAB_NUM:
			...
			simpleAst.check_special_number(expression, true);
			break;
	...
	Expression* expression = new_expression(AST_SIMPLE_EXPRESSION_TABLE);
	...
	for (uint32_t i = 0; i < expression->table->fields.size(); i++) {
		...
		if (expression->table->constants.fields[i].key->type == AST_SIMPLE_EXPRESSION_CONSTANT && expression->table->constants.fields[i].key->constant->type == AST_SIMPLE_CONSTANT_STRING)
